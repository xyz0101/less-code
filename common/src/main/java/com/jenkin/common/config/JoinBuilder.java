...
public class JoinBuilder {
    ...
    /**
     * @param joinType  关联类型  JoinBuilder.LEFT,JoinBuilder.RIGHT,JoinBuilder.INNER
     * @param mainPoClass 主表的po的class
     * @param joinPoClass 关联表的po 的class
     * @return
     */
    public JoinBuilder join(String joinType, Class mainPoClass, Class joinPoClass) {
        String mainTable = null;
        String joinTable = null;
        NotEmptyUtils.allNotEmpty(joinType,mainPoClass,joinPoClass);
        if (mainPoClass.isAnnotationPresent(TableName.class)&&joinPoClass.isAnnotationPresent(TableName.class)) {
            mainTable = ((TableName)mainPoClass.getDeclaredAnnotation(TableName.class)).value();
            joinTable = ((TableName)joinPoClass.getDeclaredAnnotation(TableName.class)).value();
        }
        NotEmptyUtils.allNotEmpty(joinType,mainPoClass,joinPoClass);
        this.joinTable = joinTable;
        this.mainTable = mainTable;
