...
public class FileClientConfiguration {
    ...
    public OSS ossClient() {
        log.info("初始化oss");
        // 创建OSSClient实例。
        return new OSSClientBuilder().build(fileProperties.getEndpoint(), fileProperties.getAccessKey(), fileProperties.getSecretKey());
    }
    ...
    public COSClient cosClient() {
        ...
        String secretId = fileProperties.getAccessKey();
        String secretKey = fileProperties.getSecretKey();
        COSCredentials cred = new BasicCOSCredentials(secretId, secretKey);
        // 2 设置 bucket 的区域, COS 地域的简称请参照 https://cloud.tencent.com/document/product/436/6224
        // clientConfig 中包含了设置 region, https(默认 http), 超时, 代理等 set 方法, 使用可参见源码或者常见问题 Java SDK 部分。
        Region region = new Region(fileProperties.getEndpoint());
